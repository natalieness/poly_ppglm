...
#%%% move this to an analysis script 
def spikes_to_neuron_df(df: pd.DataFrame) -> pd.DataFrame:
    return (
        df.groupby(["neuron", "neuron_idx"])["time_s"]
        .apply(list)
        .reset_index()
...
spks      = spikes_to_neuron_df(spikes_df)
flat_spks = spikes_to_neuron_df(flat_spikes_df)

spks["n_spikes"] = spks["spike_times"].apply(len)
flat_spks["n_spikes"] = flat_spks["spike_times"].apply(len)
spks['average rate'] = spks['n_spikes'] / (input_end - input_start)
flat_spks['average rate'] = flat_spks['n_spikes'] / (input_end - input_start)
spks['first_spike_time'] = spks['spike_times'].apply(lambda x: x[0] if len(x) > 0 else np.nan)
flat_spks['first_spike_time'] = flat_spks['spike_times'].apply(lambda x: x[0] if len(x) > 0 else np.nan)

spks = spks.sort_values(by='first_spike_time', ascending=True).reset_index(drop=True)
flat_spks = flat_spks.sort_values(by='first_spike_time', ascending=True).reset_index(drop=True)

#%%
print(f'With global syn rate: {global_syn_weight}, and p release: {global_p_release}')
print('N neurons fired (Poly / Flat):', (spks.shape[0], flat_spks.shape[0]))
print('highest firing rate:', spks['average rate'].max(), flat_spks['average rate'].max())


#%% for analysis get neuron names etc
...
#%% 
spks['name'] = spks['neuron'].map(skid_to_name)
spks['celltype'] = spks['neuron'].map(skid_to_celltype)
flat_spks['name'] = flat_spks['neuron'].map(skid_to_name)
flat_spks['celltype'] = flat_spks['neuron'].map(skid_to_celltype)

#%% plot activity of PNs

pns = neuron_details[neuron_details["name"].str.startswith('42') & neuron_details["name"].str.contains('PN')]["skeleton_id"].tolist()

tb = 0.05 
start = 0
end = 5.0
bins = np.arange(start, end + tb, tb)
spks['spk_hist'] = spks['spike_times'].apply(lambda times: np.histogram(times, bins=bins)[0])
flat_spks['spk_hist'] = flat_spks['spike_times'].apply(lambda times: np.histogram(times, bins=bins)[0])

pn_spks = spks[spks["neuron"].isin(pns)]
pn_flat_spks = flat_spks[flat_spks["neuron"].isin(pns)]
fig, ax = plt.subplots(1, 2, figsize=(10, 4), sharey=True)
for _, row in pn_spks.iterrows():
    ax[0].plot(bins[:-1], row['spk_hist'], label=row['neuron'])
for _, row in pn_flat_spks.iterrows():
    ax[1].plot(bins[:-1], row['spk_hist'], label=row['neuron'])
ax[0].set_title('Polyadic')
ax[1].set_title('Flattened')
ax[0].set_xlabel('Time (s)')
ax[1].set_xlabel('Time (s)')
ax[0].set_ylabel('Spike count per bin')
